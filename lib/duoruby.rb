...
module Duoruby
  ...
  def sign_request(ikey, skey, akey, username)
    return Duoruby::ERR_USER if not username or username.to_s.length == 0
    return Duoruby::ERR_USER if username.include? '|'
    return Duoruby::ERR_IKEY if not ikey or ikey.to_s.length != Duoruby::IKEY_LEN
    return Duoruby::ERR_SKEY if not skey or skey.to_s.length != Duoruby::SKEY_LEN
    return Duoruby::ERR_AKEY if not akey or akey.to_s.length < Duoruby::AKEY_LEN

    vals = [username, ikey]

    duo_sig = sign_vals(skey, vals, Duoruby::DUO_PREFIX, Duoruby::DUO_EXPIRE)
    app_sig = sign_vals(akey, vals, Duoruby::APP_PREFIX, Duoruby::APP_EXPIRE)
  ...
  def verify_response(ikey, skey, akey, sig_response)
    begin
      ...
      auth_user = parse_vals(skey, auth_sig, Duoruby::AUTH_PREFIX, ikey)
      app_user = parse_vals(akey, app_sig, Duoruby::APP_PREFIX, ikey)
    rescue
      return nil
    end
